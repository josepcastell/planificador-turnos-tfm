...
Regla del domini (acordada amb la cap del servei):
...
class TestRepresentativeKey:
    ...
    def test_doubled_picks_pres_position_1(self):
        ...
        sk_pos1 = ("2026-01-06", "MATI", "RM_HUB", "PRESENCIAL", "NORMAL", 1)
        sk_pos2 = ("2026-01-06", "MATI", "RM_HUB", "NO_PRESENCIAL", "NORMAL", 2)
...
class TestStructuralCouplingDoubled:
    ...
    def _build(self, slot_links):
        # Setup:
        #   - RM_HUB doblat (pos 1 PRES, pos 2 NP) a MATI dia D.
        #   - RM_DIR no doblat (PRES pos 1) a MATI dia D.
        #   - Link: (RM_DIR, RM_HUB)
        # 3 facultatius: cal almenys 3 perquè cada slot (3) tingui un
        # facultatiu diferent (max 1 màquina/franja per persona).
        model = cp_model.CpModel()
        rows = [
            _Row("2026-01-06", "MATI", "RM_HUB", "PRESENCIAL", position=1),
            _Row("2026-01-06", "MATI", "RM_HUB", "NO_PRESENCIAL", position=2),
            _Row("2026-01-06", "MATI", "RM_DIR", "PRESENCIAL", position=1),
        ]
        ...
        # Linking: només la pos PRES es lliga
    ...
    def test_pres_pos1_coupled_with_partner(self):
        # ALICE a RM_HUB pos 1 PRES => ALICE a RM_DIR (forçat pel link).
        model, x, rows = self._build([("RM_DIR", "RM_HUB")])
        sk_hub_pos1 = _make_slot_key(rows[0])
        sk_dir = _make_slot_key(rows[2])
        model.Add(x[("ALICE", sk_hub_pos1)] == 1)
        # El link força: ALICE també està a RM_DIR.
        solver = cp_model.CpSolver()
        status = solver.Solve(model)
    ...
    def test_np_pos2_not_coupled_with_partner(self):
        # BOB cobrint RM_HUB pos 2 NP NO ha d'estar obligat a RM_DIR.
        # La cobertura assigna RM_DIR a algú (ALICE), però BOB no.
        model, x, rows = self._build([("RM_DIR", "RM_HUB")])
        sk_hub_pos2 = _make_slot_key(rows[1])
        sk_dir = _make_slot_key(rows[2])
        # Forcem BOB a pos 2.
        model.Add(x[("BOB", sk_hub_pos2)] == 1)
        # Forcem ALICE a RM_DIR (perquè la cobertura es compleixi).
        model.Add(x[("ALICE", sk_dir)] == 1)
        # El link PRES també requereix Alice a RM_HUB pos 1 (PRES); ho farem
        ...
        # haver d'estar a RM_DIR (no està lligat).
        solver = cp_model.CpSolver()
        status = solver.Solve(model)
        assert status in (cp_model.OPTIMAL, cp_model.FEASIBLE)
        # Verifiquem que BOB no està a RM_DIR (no se li ha forçat).
    ...
    def test_without_link_no_coupling(self):
        ...
        sk_hub_pos1 = _make_slot_key(rows[0])
        sk_dir = _make_slot_key(rows[2])
        # Forcem Alice a RM_HUB pos 1 i Bob (un altre) a RM_DIR.
        model.Add(x[("ALICE", sk_hub_pos1)] == 1)
