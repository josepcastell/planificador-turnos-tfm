...
class TestAddPresWeekdaySoft:
    def test_no_map_returns_zero(self):
        ...
        sk = ("2026-01-05", "MATI", "TC_A", "NO_PRESENCIAL", "NORMAL", 1)
    ...
    def test_pres_not_penalized(self):
        ...
        model = cp_model.CpModel()
        sk = ("2026-01-05", "MATI", "TC_A", "PRESENCIAL", "NORMAL", 1)
        x = {("P1", sk): model.NewBoolVar("x")}
        model.Add(x[("P1", sk)] == 1)
        total = _add_pres_weekday_soft(
            model, x, [sk], {"P1": {"2026-01-05"}},
    ...
    def test_review_excluded(self):
        ...
        sk_normal = ("2026-01-05", "MATI", "TC_A", "NO_PRESENCIAL", "NORMAL", 1)
        x = {
        ...
        solver = cp_model.CpSolver()
        solver.Solve(model)
        # Només el TC_A compta (revisió fora).
        assert solver.Value(total) == 1

    def test_other_day_not_penalized(self):
        model = cp_model.CpModel()
        sk = ("2026-01-06", "MATI", "TC_A", "NO_PRESENCIAL", "NORMAL", 1)
        x = {("P1", sk): model.NewBoolVar("x")}
        model.Add(x[("P1", sk)] == 1)
        total = _add_pres_weekday_soft(
            model, x, [sk], {"P1": {"2026-01-05"}},
