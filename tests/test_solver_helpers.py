...
class TestStabilityBySlot:
    ...
    def test_maps_slot_to_professional(self):
        sk = ("2026-01-06", "MATI", "RM_A", "PRESENCIAL", "NORMAL", 1)
        df = pd.DataFrame({
            "day": ["2026-01-06"],
            "slot_id": ["RM_A"],
            "professional": ["P1"],
...
class TestAddUnavailabilityConstraints:
    ...
    def test_blocks_only_matching_day(self):
        model = cp_model.CpModel()
        sk_match = ("2026-01-06", "MATI", "RM_A", "PRESENCIAL", "NORMAL", 1)
        sk_other = ("2026-01-07", "MATI", "RM_A", "PRESENCIAL", "NORMAL", 1)
        x = {
...
class TestNormSet:
    ...
    def test_strip_and_upper(self):
        assert _norm_set([" rm_a ", "Tc3", "REV X"]) == {"RM_A", "TC3", "REV X"}
