...
class TestCascadeRenameSlotIdInFile:
    def test_updates_csv(self, tmp_path):
        ...
        pd.DataFrame({
            ...
            "slot_id": ["TC_A", "TC_A", "RM_A"],
            "allowed": [1, 1, 1],
        }).to_csv(p, index=False)
        n = cascade_rename_slot_id_in_file(p, "TC_A", "TC4")
        assert n == 2
        result = pd.read_csv(p)
        assert sorted(result["slot_id"].tolist()) == ["RM_A", "TC4", "TC4"]
...
class TestCascadeRenameSlotIdMultiFile:
    ...
    def test_cascades_to_all_files(self, _make_data):
        n = cascade_rename_slot_id("TC_A", "TC4", year=2026)
        assert n == 3  # 1 fila per fitxer
        ...
        for rel in (
            ...
            assert "TC_A" not in df["slot_id"].astype(str).tolist()
