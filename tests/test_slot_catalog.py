...
def test_default_catalog_no_templates_is_empty():
    # Sense plantilles, el catàleg per defecte és BUIT: el programa públic no
    # incrusta cap nom de màquina real; cada servei crea les seves activitats.
...
def test_default_catalog_preserves_existing_template_state():
    weekday = pd.DataFrame([{"slot_id": "TC_B"}, {"slot_id": "RM_A"}])
    weekend = pd.DataFrame([{"slot_id": "TC_RM_A"}])
    df = default_slot_catalog(weekday_templates_df=weekday, weekend_templates_df=weekend)

    # Slots present in templates → marked
    assert bool(df[df["slot_id"] == "TC_B"].iloc[0]["weekday"]) is True
    assert bool(df[df["slot_id"] == "TC_RM_A"].iloc[0]["weekend"]) is True
    ...
    assert "TC_RM_A" in df["slot_id"].tolist()
...
def test_load_handles_legacy_applies_to_column(tmp_path: Path):
    ...
    tc = df[df["slot_id"] == "TC_B"].iloc[0]
    rm = df[df["slot_id"] == "RM_A"].iloc[0]
...
def test_filters_by_scope():
    df = pd.DataFrame(
        [
            {"slot_id": "TC_B", "weekday": True, "weekend": False, "notes": ""},
            {"slot_id": "RM_A", "weekday": True, "weekend": True, "notes": ""},
            {"slot_id": "RM3T", "weekday": False, "weekend": True, "notes": ""},
        ]
    )
    assert weekday_slot_ids(df) == ["RM_A", "TC_B"]
    assert weekend_slot_ids(df) == ["RM3T", "RM_A"]
...
def test_seed_creates_file_when_missing(tmp_path: Path):
    ...
    assert path.exists()
    # El catàleg llavor és BUIT (cap dada real), però amb les columnes correctes.
    assert df.empty
    assert list(df.columns) == SLOT_CATALOG_COLUMNS
...
def test_sync_weekday_drops_unknown_slots():
    ...
    templates = pd.DataFrame(
        [
            {"weekday_name": "MONDAY", "franja": "MATI", "slot_id": "TC_B",
    ...
    assert "TC_B" in set(out["slot_id"])
