...
class TestFallbackEligibilityHard:
    def test_tld_forbidden_when_allowed_0(self):
        # Eligibility map diu (TLD, URG_DIR, allowed=0). El solver NO pot
        # assignar TLD a URG_DIR.
        model = cp_model.CpModel()
        rows = [_Row("2026-01-06", "MATI", "URG_DIR", "PRESENCIAL")]
    ...
    def test_tld_allowed_when_allowed_1(self):
        # Eligibility map diu (TLD, URG_DIR, allowed=1). TLD pot fer-ho.
        model = cp_model.CpModel()
        rows = [_Row("2026-01-06", "MATI", "URG_DIR", "PRESENCIAL")]
    ...
    def test_tld_default_allowed_when_no_entry(self):
        ...
        model = cp_model.CpModel()
        rows = [_Row("2026-01-06", "MATI", "RM_HUB", "PRESENCIAL")]
        x = _build_x(model, ["TLD"], rows)
        slot_keys = [_make_slot_key(r) for r in rows]
        elig = pd.DataFrame({
            "professional_id": ["TLD"],
            "slot_id": ["URG_DIR"],  # Entrada diferent
            "allowed": [0],
        })
        _add_fallback_eligibility_hard(
            model, x, fallback_professionals={"TLD"},
    ...
    def test_regular_not_affected(self):
        ...
        # ALICE pot assignar-se a URG_DIR encara que tingui allowed=0 al map
    ...
    def test_no_fallback_no_op(self):
        ...
        slot_keys = [_make_slot_key(r) for r in rows]
        elig = pd.DataFrame({
            "professional_id": ["TLD"], "slot_id": ["RM_HUB"], "allowed": [0],
        })
        _add_fallback_eligibility_hard(
    ...
    def test_blocks_all_positions_of_same_slot_id(self):
        ...
        status = solver.Solve(model)
        assert status in (cp_model.OPTIMAL, cp_model.FEASIBLE)
        # TLD no pot estar a cap posicio de RM_HUB.
        for sk in slot_keys:
            assert solver.Value(x[("TLD", sk)]) == 0
