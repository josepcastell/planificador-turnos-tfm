...
def build_unavailability_layer(
    ...
    if not guards_df_raw.empty:
        # post_guard_free → dia després d'una guàrdia: dia BUIT DEL TOT. Es
        # bloqueja TOTA assignació (PRESENCIAL i NO_PRESENCIAL) a qualsevol
        # franja, perquè el facultatiu de postguàrdia descansa i no se li
        # assigna cap activitat. (`presentiality=""` = comodí → bloqueja
        # totes les presencialitats; vegeu `_add_unavailability_constraints`.)
        ...
        if not post_guard_base.empty:
            ...
            for _franja in ("MATI", "TARDA", "NIT"):
                ...
                sub["franja"] = _franja
                sub["presentiality"] = ""   # buit = bloqueja PRES i NP
                parts.append(sub)
