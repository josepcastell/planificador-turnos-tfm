...
def build_and_solve_demo(data: dict, stability_assignments=None):
    ...
    # `absent_days_by_prof` drives the weekly quota: days here are excluded
    # from the active-days count. Hard slot blocking happens separately via
    # _add_unavailability_constraints. Rows whose reason is QUOTA-NEUTRAL
    # stay hard-blocked però NO redueixen la quota setmanal:
    ...
    #     compta com a dia laborable; al MATÍ el facultatiu hi fa
    #     teletreball (NP) preferentment.
    #   - "post_guard_free": dia de postguàrdia. Les PRES estan
    #     bloquejades (via unavailability), però els NP segueixen
    #     disponibles. Si el solver flipa un NP→PRES, és la "presencial
    #     de postguàrdia" i ha de comptar cap al target setmanal del
    #     facultatiu. Per això el dia ha de ser quota-neutral.
    # A més, recollim els (prof, dia) de guàrdia per a l'objectiu tou de
    # teletreball al matí.
    quota_neutral = {
        "non_working_weekday", "guardia_day_tarda", "post_guard_free",
    }
    absent_days_by_prof = {p: set() for p in professionals}
    ...
    if not unavailability_df.empty:
        for row in unavailability_df.itertuples(index=False):
            ...
            if reason == "guardia_day_tarda":
                guard_prof_days.add((row.professional_id, str(row.day)))
            if reason in quota_neutral:
                continue
            absent_days_by_prof[row.professional_id].add(str(row.day))
    ...
    # treballats = jornada × dies laborables PRESENTS (descomptant absències).
    # Així el target de màquines/presencials d'un facultatiu absent part del
    # mes és proporcionalment menor, no el d'un que hi és tot el mes.
    # `average_capacity_pct` (només jornada) no ho captava. Els targets
    # SETMANALS ja ho fan via eff_days; això alinea l'equitat mensual/acumulada.
    _n_working_days = sum(1 for d in unique_days if working_map.get(d, 1) == 1) or 1
    ...
    # marcar slots concrets on TLD NO ha d'anar (p.ex. URG_DIR). Risc:
    ...
    _add_structural_coupling(model, x, professionals, keys_by_day, slot_links=slot_links,
