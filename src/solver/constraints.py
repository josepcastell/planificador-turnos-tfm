...
def _build_machine_term_specs(
    keys_by_day, review_slots, slot_links=None,
    ...
    slot_links: optional list of (slot_a, slot_b) pairs whose slots are
    treated as a single coupled machine when both have an instance that day.
...
def _add_structural_coupling(model, x, professionals, keys_by_day, slot_links=None,
                             enforce_lit=None):
    """Per cada parella d'slots vinculats, el mateix facultatiu cobreix el
    slot representatiu de tots dos cada dia (per franja). Funciona encara
    que algun slot estigui doblat."""
    if not slot_links:
        return
    for day_keys in keys_by_day.values():
        franges = {sk[1] for sk in day_keys}
        for slot_a, slot_b in slot_links:
