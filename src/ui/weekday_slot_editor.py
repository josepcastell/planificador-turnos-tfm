...
def _presential_slots_for(
    templates_df: pd.DataFrame, weekday_name: str, franja: str
) -> list[str]:
    """Màquines PRESENCIALS presents en un (dia-setmana, franja) concret del
    template. Són les candidates a vincular-se en un bloc (el representant
    del bloc, que el solver acobla, sempre és presencial)."""
    ...
    df = templates_df
    mask = (
        (df["weekday_name"].fillna("").astype(str).str.strip().str.upper() == wd)
        & (df["franja"].fillna("").astype(str).str.strip().str.upper() == fr)
        & (
            df["presentiality"].fillna("").astype(str).str.strip().str.upper()
            == "PRESENCIAL"
        )
    )
    return sorted(
        {
            str(s).strip().upper()
            for s in df.loc[mask, "slot_id"].fillna("")
            if str(s).strip()
        }
    )


def _render_linked_groups_editor(
    templates_df: pd.DataFrame,
    weekly_templates_path: Path,
    invalidate_after_work_slot_change: Callable[[], None],
) -> None:
    """Vinculador de BLOCS de màquines per (dia-setmana, franja): permet
    definir-ne **més d'un** i és local a cada dia/franja (un dia poden estar
    vinculades i un altre no). Cada bloc = una sola persona cobreix totes les
    màquines del bloc i **compten com 1**. Diferent de doblar (1 màquina, 2
    persones)."""
    ...
    st.divider()
    with st.expander(
        f"🔗 Vincular màquines en blocs (fins a {MAX_LINKED_GROUP})",
        expanded=False,
    ):
        st.caption(
            "Vincula diverses màquines que cobreix la **mateixa persona** en "
            "aquest dia i franja: el solver hi posa el mateix facultatiu i "
            "**compten com 1** màquina. Pots definir **més d'un bloc** i és "
            "**per (dia, franja)** — un dia poden estar vinculades i un altre "
            "no. És diferent de **doblar** (doblar = 1 màquina, 2 persones)."
        )
        c_day, c_fr = st.columns(2)
        with c_day:
            wd = st.selectbox(
                "Dia",
                WEEKDAY_TEMPLATE_COLUMNS,
                key="linkgrp_wd",
                format_func=lambda v: WEEKDAY_LABELS[
                    WEEKDAY_TEMPLATE_COLUMNS.index(v)
                ],
            )
        with c_fr:
            fr = st.selectbox(
                ...
                FRANJA_OPTIONS,
                key="linkgrp_fr",
                format_func=lambda v: FRANJA_LABELS[v],
            )
        ...
        if groups:
            for gi, g in enumerate(groups):
                ...
                if col2.button(
                    ...
                    new_df = clear_linked_group_in_template(
                        templates_df, wd, fr, g[0]
                    )
                    save_weekly_slot_templates(new_df, weekly_templates_path)
                    invalidate_after_work_slot_change()
                    st.toast("Bloc desvinculat", icon="🔓")
                    st.rerun()
        else:
            st.caption("Cap bloc en aquest dia i franja.")

        present = _presential_slots_for(templates_df, wd, fr)
        if len(present) < 2:
            st.info(
                "Calen com a mínim 2 màquines presencials en aquest dia i "
                "franja per poder vincular-les."
            )
            return
        sel = st.multiselect(
            "Màquines del bloc",
            options=present,
            key=f"linkgrp_sel_{wd}_{fr}",
            help=f"Tria de 2 a {MAX_LINKED_GROUP} màquines presencials.",
        )
        too_many = len(sel) > MAX_LINKED_GROUP
        if too_many:
            st.warning(f"Màxim {MAX_LINKED_GROUP} màquines per bloc.")
        if st.button(
            "Vincular com a bloc",
            type="primary",
            disabled=(len(sel) < 2 or too_many),
            key=f"linkgrp_add_{wd}_{fr}",
        ):
            new_df = set_linked_group_in_template(templates_df, wd, fr, sel)
            save_weekly_slot_templates(new_df, weekly_templates_path)
            invalidate_after_work_slot_change()
            st.toast("Bloc vinculat", icon="🔗")
            st.rerun()
