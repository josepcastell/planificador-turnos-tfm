...
def render_allowed_areas_editor(
    ...
    Si conté valors (p.ex. HUB;DELTA), el solver bloqueja qualsevol
...
def render_comite_editor(
    ...
    with st.form("quick_add_comite", clear_on_submit=True):
        ...
        with cols_form[2]:
            qa_type = st.selectbox(
                "lloc", options=COMITE_TYPES, index=0, label_visibility="collapsed",
            )
        with cols_form[3]:
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "comite_type": st.column_config.SelectboxColumn(
                "Lloc", options=COMITE_TYPES, required=True,
                help="HUB o DIR — defineix el grup de màquines a forçar aquell dia.",
            ),
