...
_AREA_TOKENS = {"DIR", "HUB", "DELTA"}
_FAMILY_TOKENS = {"TC", "RM"}


def _infer_area_family(slot_id: str) -> tuple[str, str]:
    """Detecta àrea (DIR/HUB/DELTA) i família (TC/RM) a partir del nom de
    l'slot. Patrons típics: TC_DIR, TC1_HUB, RM_DIR, REV_TC, REV_RM."""
    area = ""
    family = ""
    for token in str(slot_id).strip().upper().split("_"):
        if not token:
            continue
        if not area and token in _AREA_TOKENS:
            area = token
    ...
    st.dataframe(
        ...
        column_config={
            "slot_id": st.column_config.TextColumn(
                "Activitat",
                help="Identificador en majúscules (ex. TC_DIR). Sufix _HUB / "
                "_DIR determina el grup de comitè.",
            ),
            ...
            "area": st.column_config.TextColumn(
                "Lloc",
                help="Localització (DIR/HUB/DELTA o lliure). DIR/HUB s'usen "
                "per als grups de comitè i al PDF.",
            ),
