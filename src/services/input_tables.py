...
DEFAULT_WEEKDAY_ELIGIBILITY_SLOTS = {
    "3T_DIR",
    "1.5T_DIR",
    "TC_DIR",
    "RM_HUB",
...
def _coerce_areas_column(series_or_value):
    """Normalitza un valor de `allowed_areas` ("HUB;DELTA") a string
...
def _migrate_double_tc_urg(df: pd.DataFrame) -> pd.DataFrame:
    """Backwards-compat: if df has legacy `double_tc_urg` and not `doubled_machines`,
    populate the new column from the boolean."""
    out = df.copy()
    if "doubled_machines" not in out.columns:
        if "double_tc_urg" in out.columns:
            legacy = out["double_tc_urg"].apply(normalize_bool_value)
            out["doubled_machines"] = legacy.map(lambda v: "TC_URG" if v else "")
        else:
...
def professional_scope_df(
    ...
    weekday = _migrate_double_tc_urg(weekday_professionals_df)
