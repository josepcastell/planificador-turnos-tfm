...
_FILL_AREA_DIR = PatternFill("solid", fgColor="DBEAFE")
_FILL_AREA_HUB = PatternFill("solid", fgColor="DCFCE7")
_FILL_AREA_DEL = PatternFill("solid", fgColor="EDE9FE")
...
def _area_fill(area: str) -> PatternFill:
    area = (area or "").upper()
    if area == "DIR":
        return _FILL_AREA_DIR
    if area == "HUB":
        return _FILL_AREA_HUB
    if area == "DEL":
        return _FILL_AREA_DEL
...
def _area_sort_key(area: str) -> int:
    """Ordre de les àrees al PDF: DIR, HUB, DEL, ALTRES."""
    order = {"DIR": 0, "HUB": 1, "DEL": 2}
    return order.get(str(area).upper(), 99)
